...
<div class="row">
    ...
    <div class="col-lg-3 col-6">
        <div class="small-box bg-danger">
            ...
            <a href="{{ route('medicines.index') }}?filter=low_stock" class="small-box-footer">Check Now <i class="fas fa-arrow-circle-right"></i></a>
        </div>
    </div>
</div>
            ...
            <div class="card-body p-0">
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <span>{{ $med->name }}</span>
                        <span class="badge badge-warning">{{ \Carbon\Carbon::parse($med->expiry_date)->format('d M Y') }}</span>
                    </li>
                    @empty
        ...
        <div class="card card-danger">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-hover mb-0">
                    <thead><tr><th>Medicine</th><th>Stock</th><th>Reorder Level</th></tr></thead>
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="3" class="text-muted text-center">All stock levels are fine!</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
new Chart(ctx, {
    type: 'bar',
    data: {
        labels: {!! $monthlySales->pluck('month')->map(fn($m) => '"'.date('F', mktime(0,0,0,$m,1)).'"')->join(',') !!},
        datasets: [{
            label: 'Revenue (৳)',
            data: {!! $monthlySales->pluck('total')->join(',') !!},
